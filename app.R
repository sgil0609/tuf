...
init_database <- function() {
  ...
  dbExecute(con, "
    CREATE TABLE IF NOT EXISTS drafter_passwords (
      drafter_id INTEGER PRIMARY KEY,
      password VARCHAR
    )
  ")
  ...
  if (nrow(dbGetQuery(con, "SELECT * FROM drafter_passwords")) == 0) {
    insert_passwords(con)
  }
...
insert_passwords <- function(con) {
  ...
  dbWriteTable(con, "drafter_passwords", passwords, append = TRUE)
}

# Authenticate drafter
authenticate_drafter <- function(drafter_id, password) {
  con <- dbConnect(duckdb::duckdb(), "fantasy_draft.db")
  
  result <- dbGetQuery(con, "
    SELECT password FROM drafter_passwords WHERE drafter_id = ?
  ", list(drafter_id))
  
  dbDisconnect(con)
  
  if (nrow(result) > 0 && result$password == password) {
    return(TRUE)
  } else {
    return(FALSE)
  }
...
ui <- dashboardPage(
  ...
  dashboardBody(
    ...
    tags$script(HTML("
      // LocalStorage functions for better persistence
      function setSession(drafter_id) {
        localStorage.setItem('fantasy_draft_user', drafter_id);
        localStorage.setItem('fantasy_draft_timestamp', Date.now());
      }
      
      function getSession() {
        var drafter_id = localStorage.getItem('fantasy_draft_user');
        var timestamp = localStorage.getItem('fantasy_draft_timestamp');
        
        // Check if session is less than 7 days old
        if (drafter_id && timestamp) {
      ...
      function clearSession() {
        localStorage.removeItem('fantasy_draft_user');
        localStorage.removeItem('fantasy_draft_timestamp');
      }
      
      // Auto-login on page load
      $(document).ready(function() {
        setTimeout(function() {
          var loggedInDrafter = getSession();
          if (loggedInDrafter) {
            Shiny.setInputValue('restore_session', loggedInDrafter, {priority: 'event'});
          }
    ...
    conditionalPanel(
      ...
      fluidRow(
        column(4, offset = 4,
          box(
            title = "Drafter Login", status = "primary", solidHeader = TRUE, width = 12,
            selectInput("login_drafter", "Select Drafter:",
              choices = setNames(1:10, paste("Drafter", 1:10)),
              selected = 1
            ),
            passwordInput("login_password", "Password:", placeholder = "Enter your password"),
            br(),
            actionButton("login_btn", "Login", class = "btn-success btn-block"),
            br(),
            div(id = "login_error", style = "color: red; text-align: center;")
          )
        )
      )
    ),
        ...
        tabItem(tabName = "draft",
          ...
          fluidRow(
            ...
            box(
              ...
              # Draft permission message
              conditionalPanel(
                condition = "output.can_draft == false",
                div(class = "alert alert-warning",
                  h4("Not Your Turn"),
                  p("You can view the draft board, but you can only make picks when it's your turn.")
                )
              ),
...
server <- function(input, output, session) {
  ...
  values <- reactiveValues(
    ...
    authenticated = FALSE,
    logged_in_drafter = NULL
  )
  
  # Handle session restoration from cookie
  observeEvent(input$restore_session, {
    drafter_id <- as.numeric(input$restore_session)
    if (!is.na(drafter_id) && drafter_id >= 1 && drafter_id <= 10) {
      values$authenticated <- TRUE
      values$logged_in_drafter <- drafter_id
      session$userData$authenticated <- TRUE
      session$userData$logged_in_drafter <- drafter_id
    }
  })
  ...
  output$can_draft <- reactive({
    if (!values$authenticated) return(FALSE)
    state <- values$draft_state
    return(values$logged_in_drafter == state$current_drafter)
  })
  outputOptions(output, "can_draft", suspendWhenHidden = FALSE)
  
  output$logged_in_user <- renderText({
    if (values$authenticated) {
      paste("Logged in as: Drafter", values$logged_in_drafter)
    } else {
      ""
    }
  })
  ...
  observeEvent(input$login_btn, {
    drafter_id <- as.numeric(input$login_drafter)
    password <- input$login_password
    
    if (authenticate_drafter(drafter_id, password)) {
      values$authenticated <- TRUE
      values$logged_in_drafter <- drafter_id
      
      # Store authentication state in session for persistence
      session$userData$authenticated <- TRUE
      session$userData$logged_in_drafter <- drafter_id
      ...
      shinyjs::runjs(paste0("setSession('", drafter_id, "');"))
      
      showNotification("Login successful!", type = "message", duration = 3)
    } else {
      shinyjs::html("login_error", "Invalid password. Please try again.")
      updateTextInput(session, "login_password", value = "")
    }
  })
  ...
  observeEvent(input$logout, {
    ...
    values$logged_in_drafter <- NULL
    
    # Clear authentication state from session
    session$userData$authenticated <- FALSE
    session$userData$logged_in_drafter <- NULL
    
    # Clear localStorage session
    shinyjs::runjs("clearSession();")
  ...
  observeEvent(input$players_table_rows_selected, {
    if (length(input$players_table_rows_selected) > 0) {
      # Check if user is authenticated and it's their turn
      if (!values$authenticated) {
        showNotification("You must be logged in to make picks.", type = "warning", duration = 3)
      ...
      if (values$logged_in_drafter != current_drafter) {
        showNotification("It's not your turn to draft.", type = "warning", duration = 3)
        DT::selectRows(DT::dataTableProxy("players_table"), NULL)
        return()
      }
